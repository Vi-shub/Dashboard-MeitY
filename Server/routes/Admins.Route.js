...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (admin.length > 0) {
      ...
      const utf8Password = Buffer.from(password, "utf-8").toString("utf-8");
      const utf8StoredPassword = Buffer.from(
        admin[0].password,
        "utf-8"
      ).toString("utf-8");

      bcrypt.compare(utf8Password, utf8StoredPassword, (err, results) => {
